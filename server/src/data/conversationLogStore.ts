...
export async function resolveConversation(websiteId: string, id: string): Promise<boolean> {
  ...
  const { rowCount } = await getPool().query(`UPDATE conversation_logs SET resolved = TRUE WHERE id = $1 AND website_id = $2`, [id, websiteId]);
  return (rowCount ?? 0) > 0;
}
...
export async function listConversations(websiteId: string, limit = 100): Promise<ConversationLogEntry[]> {
  if (!isChatLogEnabled()) return [];
  const { rows } = await getPool().query(`SELECT * FROM conversation_logs WHERE website_id = $1 ORDER BY created_at DESC LIMIT $2`, [
    websiteId,
    limit,
  ]);
  return rows.map(rowToEntry);
